...
export function registerObserveCommand(program: Command): void {
  program
    ...
    .option('--provider <provider>', 'LLM provider: "claude" (default) or "ollama"')
    .option('--ollama-url <url>', 'Ollama API URL (default: OLLAMA_URL env or http://localhost:11434)')
    ...
    .action(async (logPath: string, opts: {
      ...
      const validProviders = ['claude', 'ollama'];
      if (opts.provider && !validProviders.includes(opts.provider)) {
        exitWithError(`--provider must be one of: ${validProviders.join(', ')}`, opts.json);
      }
