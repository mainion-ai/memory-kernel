...
| `mk reflect -d <dir> [--backfill-human-edits] [--json]` | Consolidate: dedup, expire, promote, detect conflicts. `--backfill-human-edits` detects off-band filesystem edits (content-diff vs. the last recorded snapshot, or an `updated_at`-newer-than-last-event heuristic) and emits synthetic `human_edit` events for clearly-scattered ones — bulk same-second `mk doctor --fix` migration clusters are skipped (#247) |
...
| `mk grounding -d <dir> [--json] [--prior-threshold N] [--grounding-threshold N] [--actionable-only] [--include-all] [--apply [--dry-run] [--override]]` | **Advisory + read-only by default.** Reconcile each atom's stated confidence (the *prior*) against a usage `grounding_score` derived purely from the event log (recency + read-frequency, discounted per detected conflict; never-read floors at 0.01), then bin into a 2×2 `prior × grounding` quadrant — **review** (confident but unused), **promote** (cautious but well-used), **noise**, **well-grounded**. Plain `mk grounding` **writes no atom files**. **`--apply`** (#364) writes a reconciled confidence back to the actionable review/promote atoms (`reconciled = clamp(prior + α·(grounding − prior))`, α_neg 0.08 > α_pos 0.03) and emits an `atom_reconciled` audit event per write; atoms a human has edited (`human_edit`, #247) are **skipped** unless `--override`; `--dry-run` previews |
...
| `mk edit <atom-id> -d <dir> [--dry-run] [--json]` | Open an atom in `$EDITOR` and record a provenanced `human_edit` event when the file changes (the strongest correction signal in the store — a human directly validating or correcting an atom). No-op when saved unchanged; encrypted (SECRET) atoms are not editable this way (#247) |
