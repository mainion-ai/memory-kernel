...
- **Skill seed-atom convention:** `/mk-memory-setup` bundles its lifecycle seed atoms at `skills/mk-memory-setup/seed-atoms/lifecycle/` (one markdown body per atom) and seeds them via the bundled `seed-atoms/seed-lifecycle.sh` script (called from Step 6b in `SKILL.md`). The eight atoms (7 procedures + 1 constraint) mirror sections of `docs/agent-session-loop.md` — keep them in sync when either side changes. Re-seeding requires moving the stale `ENTITIES/<id>.md` to `ARCHIVE/` first, since `generateAtomId()` always appends a unique suffix.
