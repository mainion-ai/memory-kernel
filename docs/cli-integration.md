...
**Use `mk init --cron` for the production wrapper.** It generates a hardened script (#303) that encodes the field lessons: a self-contained `PATH` (incl. the agent's `MK_BIN`, #345), a **KNOWLEDGE auto-observe step** (#256, below), **fail-soft** guards on every step (no bare `set -e` — one non-fatal failure can't silently kill the whole sync), the `reindex --embed` step, and the `mk doctor` self-canary. Don't hand-roll the bare commands above for an unattended cron.

> **Nightly KNOWLEDGE auto-observe (#256).** The generated wrapper scans the store's `KNOWLEDGE/` tree for `.md` docs changed since the last run and `mk observe … --mode document`s each (then `reflect` turns them into atoms) — so research/design docs dropped into `KNOWLEDGE/` become memory automatically (the runtime half of the #244 canonical dir). It **skips `KNOWLEDGE/draft/`** (work-in-progress) and the scaffolded `KNOWLEDGE/README.md`, and is **idempotent** via a `.knowledge-manifest` (per-doc mtime) so unchanged docs aren't re-observed. A doc whose `observe` fails is retried next run (its old mtime is carried forward).
