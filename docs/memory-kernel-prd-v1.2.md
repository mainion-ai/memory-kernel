...
- A full **LoCoMo-competitive conversational memory reasoning system** (semantic retrieval + temporal reconstruction + event summarization + multimodal handling). This PRD covers the **memory substrate**; a “memory intelligence layer” is vNext.
- Perfect semantic recall in all domains (v1 focuses on correctness, lifecycle, and system semantics; semantic/vector retrieval is optional and treated as a plugin).
...
Benchmarks are treated as **system-level evals** (reporting + trend tracking). In v1, the success criterion is **“harness runnable + baseline recorded”**, not “competitive score.”

- **v1 (report-only readiness):**
  - Harness runner exists (pinned environment) and produces a repeatable baseline report.
  - Synthetic compaction-loss and deterministic recall tests act as PR gates.
- **vNext (score targets):**
  - Introduce minimum score thresholds and regression budgets once the memory intelligence layer (semantic + temporal + summarization) exists.
...
- Fast-start option: adopt `memory-kernel` (npm) as the baseline v0.1 implementation; extend toward v1.0 features.
...
- Merge semantics (CRDT-backed shared views **or** event-log union + deterministic reducers)
- Conflict surfacing + resolver workflow hooks
- MCP server exposing memory tools/resources
- **Task-aware recall**: `RecallQuery.task` must influence retrieval/ranking (minimum: lexical/FTS-based ranking; embeddings optional)
- **First-class episodes**: store/read episode/session summaries and allow episode-aware recall (by provenance, scope, and task query)
...
#### FR-2a Episode Store (session/episode artifacts)

Episodes are the **human-readable** record of “what happened over time,” kept separate from durable semantic state.

- Stores per-session or per-workstream artifacts:
  - session summaries, plans, attempted approaches, results, key tool outputs
  - (optional) raw dialogue transcripts when available/allowed
- Episodes must be referenceable via `provenance` from memory atoms and decisions.
- Recall must be able to include episodes **on demand** (by provenance, scope, and query) without forcing full episodic history into startup context.
...
- Required views:
  ...
  - `OPEN_QUESTIONS.md`
  - `ENTITIES/*.md`
- Views are derived from events via deterministic transforms (v1). Optional LLM-assisted reflection/summarization is vNext and must be schema-validated + canonicalized.
...
- Progressive disclosure:
  ...
  - conditionally load: domain/entity/decisions related to touched_paths and active links
  - episodic logs are retrieved *on-demand* only (by provenance, scope, and query)
- **Task-aware retrieval is required in v1:**
  - `RecallQuery.task` (or `query.text`) must influence retrieval and ranking.
  - Minimum implementation: lexical ranking (SQLite FTS5 or deterministic keyword scoring) over atom titles/body + key frontmatter fields.
  - Embedding/vector retrieval is optional and pluggable (v2/vNext), not required for v1 correctness.
- Retrieval modes:
  - deterministic (path/tag/id)
  - lexical / full-text (FTS or keyword scoring) **(required for v1)**
  - semantic (optional embeddings; plugin)
  - graph-based (optional relationship traversal)

#### FR-6 Reflect (curate / consolidate)

**v1 default:** deterministic (no LLM calls). LLM-assisted reflection is vNext and must be strictly schema-validated.
...
### 10.8 memory-kernel (npm / TypeScript) (near-complete v0.1 MVP)

- NPM: https://www.npmjs.com/package/memory-kernel  
- Repo: https://github.com/mainion-ai/memory-kernel  
- Summary: model-agnostic, file-first **typed atoms** (markdown + YAML frontmatter) with `status`, `confidence`, `ttl_days`, and scoped `tags`/`paths`; provides the three core ops **retain / recall / reflect**, an append-only JSONL **event log**, generated **views** (`INDEX.md`, `DECISIONS.md`, `OPEN_QUESTIONS.md`, etc.), and an optional **SQLite index** that is always rebuildable from files.
- Strengths vs this PRD: implements most of the **MVP v0.1** requirements out-of-the-box (typed atoms, TTL/promotion, token-aware recall, dedupe/expiry, CLI + SDK) and includes a practical adapter path (rendering a `CLAUDE.md` handoff for agent sessions).
- Gaps to reach full PRD scope:
  - **Multi-agent convergence:** no CRDT merge layer / conflict resolver workflow (needs `merge(remote)` + explicit `conflict` atom generation + resolver hooks).
  - **Security/policy:** classification gating + encryption-at-rest not enforced by default (needs policy engine + audits + secret handling).
  - **Replay guarantees:** event log exists, but deterministic rebuild/replay semantics must be verified and/or hardened (tests + tooling).
  - **Benchmark capability:** not “LoCoMo-ready” as a standalone system (missing semantic retrieval, episodic ingestion from raw dialogue, temporal/event graph, question-aware retrieval planning, multimodal support). Treat it as the **persistence core** under a LoCoMo-capable stack.
...
**v1 requirement:** support **task-aware** recall without embeddings.
- Implement SQLite metadata indexes (atoms, tags, paths, status, timestamps) **plus** an optional **FTS (full-text) index** over atom titles/body (FTS5).
- If SQLite is unavailable, provide a deterministic lexical fallback (keyword scoring / grep-like matching).
...
**Optional (v2/vNext):** vector index for semantic recall (pluggable).
...
### 11.6a Episodes: keep raw history out of startup context

- Store long-form session artifacts in `EPISODES/` and link them from atoms via `provenance`.
- Provide deterministic helpers:
  - `writeEpisode(session_id, summary_md, metadata)`
  - `linkEpisodeToAtom(atom_id, episode_id)`
- Recall should only include episodes when:
  - directly referenced by an atom’s provenance, **or**
  - requested via `RecallQuery.include_episodes`, **or**
  - the task/keyword query matches episode summaries (lexical/FTS).
...
- **Task-aware recall ranking determinism** (same store + same query.task → same ranked bundle)
...
- recall(task=...) → ranking changes meaningfully (lexical/FTS) and is deterministic
- episode APIs → write/read/link + recall(include_episodes) works as specified
...
- `memory-kernel` conformance smoke test: `mk init` → `mk remember` → `mk recall` → `mk reflect` → `mk reindex` → `mk doctor` (golden outputs)
...
Benchmarks are **reporting-only** in v1 unless explicitly promoted to gates.

- **v1:** focus on deterministic infrastructure tests + synthetic compaction-loss torture tests.
- **v1 readiness goal:** the benchmark harness runner can execute in a pinned environment and record a baseline (not necessarily “good scores”).
- **vNext:** introduce score thresholds/regression budgets once semantic/temporal/summarization layers exist.
...
- Weekly: benchmark baselines + trend reports (report-only in v1)
...
6. Task-aware recall implementation choice for v1: SQLite FTS5 vs deterministic keyword scoring fallback (or both)  
7. Benchmark harness packaging: do we vendor/pin LoCoMo/LongMemEval runners in a separate repo/container to avoid network dependency in CI?
