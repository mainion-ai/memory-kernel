...
## Validation Testbench (LongMemEval)

Run the mk-testbench to measure recall accuracy against the LongMemEval benchmark.
Compares mk against published baselines (BM25~38%, Dense~52%, Oracle~70%).

### Prerequisites

```bash
mk --version        # need 1.12.0+ (FTS fix — earlier versions return wrong atoms)
python3 --version   # 3.11+
```

If mk is below 1.12.0, upgrade from memory-kernel-dev:

```bash
cd ~/repos/memory-kernel-dev
git pull origin main && npm install && npm run build && sudo npm link
mk --version   # confirm 1.12.0
```

**Always reindex your store after upgrading:**

```bash
mk reindex --dir ~/repos/memory/kernel
```

### Setup

```bash
git clone git@github.com:mainion-ai/mk-testbench.git
cd mk-testbench
python3 -m venv venv && source venv/bin/activate
pip install -r requirements.txt
```

Download the dataset (278MB, one-time):

```bash
cd data && python3 -c "
from huggingface_hub import snapshot_download
snapshot_download(repo_id='xiaowu0162/LongMemEval', repo_type='dataset', local_dir='.')
" && cd ..
# File arrives without extension — rename if needed:
mv data/longmemeval_s data/longmemeval_s.json 2>/dev/null || true
python3 -c "import json; print(len(json.load(open('data/longmemeval_s.json'))), 'instances')"
# → 500 instances
```

Create `run_eval.sh` (gitignored — safe for the key):

```bash
cat > run_eval.sh << 'SCRIPT'
#!/bin/bash
cd "$(dirname "$0")"
source venv/bin/activate
export OPENAI_API_KEY="sk-proj-..."   # from ~/repos/memory-kernel-dev/.env or NanoClaw .env
exec python -u benchmarks/longmemeval/evaluate.py "$@"
SCRIPT
chmod +x run_eval.sh
```

The judge uses GPT-4o (matches original LongMemEval methodology for comparable numbers).

### Smoke test

```bash
bash run_eval.sh --dataset data/longmemeval_s.json --output /tmp/smoke.json --limit 3 --verbose
```

Expected: ~2/3 correct. If judge returns 0% — OpenAI key not loading.
...
nohup bash run_eval.sh \
  ...
  --verbose > results/baseline.log 2>&1 &
echo "PID: $!"
tail -f results/baseline.log
```

Runtime: ~4-8 hours. Results print automatically on completion.

### Interpreting results

| Question type | What a low score means |
|---|---|
| `single-session-preference` | Normal — implicit preferences don't survive fact-oriented ingestion |
| `temporal-reasoning` | Expected weak spot — mk has no temporal index |
| `multi-session` | Should be strong — turn-level chunking helps cross-session recall |

Published baselines (LongMemEval_S, GPT-4o judge):

| System | Accuracy |
|---|---|
| BM25 retrieval | ~38% |
| Dense retrieval (GTE) | ~52% |
| Oracle (full context) | ~70% |
| Full-context LLM | ~65% |
| mk 1.11.0 (broken FTS, Run 1) | 53.2% |

### Commit and coordinate

```bash
sed -i 's|^results/$|# results/ (tracked)|' .gitignore
git add results/ .gitignore
git commit -m "Add baseline results (mk vX.Y.Z)"
git push origin master
```

Then notify Taj and Mai via A2A. Tests 6 & 7 (recall relevance + differentiation) run
against multiple agent stores simultaneously — coordinate before running so all stores
get the same queries at the same time.
